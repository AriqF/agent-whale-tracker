...
async function sendAgentReply(
  ...
  const loadingMsg = await bot.sendMessage(chatId, '🔍 Mengambil data whale\\.\\.\\.', {
...
export function registerHandlers(bot: TelegramBot): void {
  ...
  bot.onText(/\/help/, (msg) => {
    ...
    bot.sendMessage(
      ...
      '*Panduan Whale Signal Agent*\n\n' +
        '*Perintah:*\n' +
        '• `/signal btc` — posisi whale BTC sekarang\n' +
        '• `/signal eth` — posisi whale ETH\n' +
        '• `/trend btc` — trend akumulasi/distribusi\n' +
  ...
  bot.onText(/\/trend(?:\s+(\w+))?/, async (msg, match) => {
    const chatId = msg.chat.id;
    if (!isAllowed(chatId)) return;
